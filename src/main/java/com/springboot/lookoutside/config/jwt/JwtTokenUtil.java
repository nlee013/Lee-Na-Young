...
public class JwtTokenUtil implements Serializable {
    ...
    public static final long JWT_TOKEN_VALIDITY = 5*24 * 60 * 60; //토큰의 기한 (5일로 설정) 120시간
    ...
    public String generateToken(String useId) {
        ...
        return doGenerateToken(claims, useId);
    }
    ...
    private String doGenerateToken(Map<String, Object> claims, String subject) {
        return Jwts.builder().setClaims(claims).setSubject(subject).setIssuedAt(new Date(System.currentTimeMillis()))
                .setExpiration(new Date(System.currentTimeMillis() + JwtProperties.EXPIRATION_TIME)) // 10분인 상태
                .signWith(SignatureAlgorithm.HS512, JwtProperties.SECRET).compact();
    }

    //validate token (토큰의 유효여부를 검사한다)
    public Boolean validateToken(String token, UserDetails userDetails) {
        final String useId = getUseIdFromToken(token);
        return (useId.equals(userDetails.getUsername()) && !isTokenExpired(token));
    }
}